...
> 简洁说明：收银台（Cashier）为前端静态站点，已集成到 `agpay-payment-api` 服务的 `wwwroot/cashier` 目录中。

---

## 1. 概述

Cashier 是嵌入在 `agpay-payment-api` 中的前端收银台页面（SPA）。部署有两种方式：

- 随 `agpay-payment-api` 镜像一起打包（推荐，在构建镜像时复制 `dist` 到 `wwwroot/cashier`）。
...
# Linux - 更新 agpay-payment-api 并重建 cashier
...
3. 构建 `agpay-payment-api` 镜像并启动服务。
...
docker compose build agpay-payment-api
docker compose up -d agpay-payment-api
```
...
- 收银台访问路径：`https://<HOST>:9819/cashier`（由 `agpay-payment-api` 提供静态资源）。
- 确保 `agpay-payment-api` 的静态文件路径正确挂载到容器中（`wwwroot/cashier`）。
...
3. 使用 `docker compose logs -f agpay-payment-api` 查看启动或静态文件错误日志。
