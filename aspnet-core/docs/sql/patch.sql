...
ALTER TABLE `t_sys_log`   
  ADD COLUMN `browser` VARCHAR(60) DEFAULT '' NOT NULL COMMENT '浏览器' AFTER `user_name`,
  ADD COLUMN `os` VARCHAR(60) DEFAULT '' NOT NULL COMMENT '操作系统' AFTER `browser`,
  ADD COLUMN `device` VARCHAR(60) DEFAULT '' NOT NULL COMMENT '设备' AFTER `os`,
  ADD COLUMN `browser_info` VARCHAR(200) DEFAULT '' NOT NULL COMMENT '浏览器信息' AFTER `device`,
  ADD COLUMN `req_method` VARCHAR(10) DEFAULT '' NOT NULL COMMENT '请求方法' AFTER `req_url`,
  ADD COLUMN `elapsed_ms` BIGINT NULL COMMENT '耗时（毫秒）' AFTER `opt_res_info`;