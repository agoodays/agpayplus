...
namespace AGooday.AgPay.Agent.Api.Controllers.Order
{
    /// <summary>
    /// 退款订单类
    /// </summary>
    ...
    public class RefundOrderController : CommonController
    {
        ...
        public async Task<IActionResult> ExportAsync(string bizType, [FromQuery] RefundOrderQueryDto dto)
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                // 将每个订单添加到工作表中
                for (int i = 0; i < refundOrders.Items.Count; i++)
                {
                    var refundOrder = refundOrders.Items[i];
                ...
                var rows = refundOrders.Items.Count + 3;
        ...
        [PermissionAuth(PermCode.AGENT.ENT_REFUND_ORDER_VIEW)]
