...
{
    public class DgPayApplymentService : AbstractApplymentService
    {
        public DgPayApplymentService(
            ILogger<DgPayApplymentService> logger,
        ...
        public override string GetIfCode() => CS.IF_CODE.DGPAY;
        ...
        public override Task<ApplymentSubmitRS> SubmitAsync(ApplymentSubmitRQ rq, MchApplyDto mchApply, IsvConfigContext isvCtx)
        {
            ...
            var isvParams = isvCtx?.GetIsvParamsByIfCode<DgPayIsvParams>(CS.IF_CODE.DGPAY);
            if (isvParams == null)
            {
                result.ErrMsg = "斗拱通道配置不存在";
                return Task.FromResult(result);
            }

            result.ErrMsg = "斗拱通道进件服务待接入，请联系技术支持";
            return Task.FromResult(result);
        }
        ...
        public override Task<ApplymentQueryRS> QueryAsync(string applyId, string channelApplyNo, string channelMchId, MchApplyDto mchApply, IsvConfigContext isvCtx)
        {
            ...
            if (isvParams == null)
            {
                result.ChannelStateDesc = "斗拱通道配置不存在";
                return Task.FromResult(result);
            }

            result.ChannelStateDesc = "斗拱通道进件服务待接入";
            return Task.FromResult(result);
        }
    }
}
