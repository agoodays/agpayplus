...
namespace AGooday.AgPay.Components.Third.Channel.DgPay
{
    ...
    public class DgPayChannelNoticeService : AbstractChannelNoticeService
    {
        ...
        public async Task<bool> VerifyParamsAsync(JObject jsonParams, PayOrderDto payOrder, MchAppConfigContext mchAppConfigContext)
        {
            ...
            {
                _logger.LogInformation("金额参数为空 [amt]={amt}", transAmt);
            ...
            {
                _logger.LogInformation("【斗拱回调】 验签失败！ 回调参数: parameter={parameter}, publicKey={publicKey}", jsonParams, publicKey);
            ...
            {
                _logger.LogInformation("订单金额与参数金额不符。 dbPayAmt={dbPayAmt}, amt={amt}, payOrderId={payOrderId}", dbPayAmt, transAmt, reqSeqId);
