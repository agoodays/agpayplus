...
namespace AGooday.AgPay.Components.Third.Channel.LcswPay
{
    ...
    public class LcswPayChannelNoticeService : AbstractChannelNoticeService
    {
        ...
        public LcswPayChannelNoticeService()
            : base()
        {
        }
        ...
        public async Task<bool> VerifyParamsAsync(JObject jsonParams, PayOrderDto payOrder, MchAppConfigContext mchAppConfigContext)
        {
            ...
            {
                _logger.LogInformation("订单ID为空 [terminalTrace]={terminalTrace}", terminalTrace);
                //_logger.LogInformation($"订单ID为空 [terminalTrace]={terminalTrace}");
                return false;
            }
            ...
            {
                _logger.LogInformation("订单金额与参数金额不符。 dbPayAmt={dbPayAmt}, totalFee={totalFee}, payOrderId={terminalTrace}", dbPayAmt, totalFee, terminalTrace);
                //_logger.LogInformation($"订单金额与参数金额不符。 dbPayAmt={dbPayAmt}, totalFee={totalFee}, payOrderId={terminalTrace}");
                return false;
            }
            return true;
        }
