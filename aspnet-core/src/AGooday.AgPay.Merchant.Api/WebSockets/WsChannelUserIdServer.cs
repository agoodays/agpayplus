    ...
    public class WsChannelUserIdServer
    {
            ...
            try
            {
                ...
                do
                {
                    ...
                    else
                    {
                        ...
                        this.SubOnlineCount();
                        _logger.LogInformation("WebSocket客户端Cid[{Cid}], AppId[{AppId}], 当前在线人数为: {OnlineClientSize}", this.Cid, this.AppId, OnlineClientSize);
                    }
                }
        ...
        public async Task SendMsgByAppAndCid(string appId, string cid, string msg)
        {
            try
            {
                ...
                if (!isExist)
                {
                    _logger.LogInformation("AppId[{AppId}] 无ws监听客户端", appId);
                    return;
                }
            ...
            {
                _logger.LogInformation(e, "推送消息时异常，AppId={AppId}", appId);
            }
        }
