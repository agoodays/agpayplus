...
namespace AGooday.AgPay.Merchant.Api.Controllers.Anon
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public ApiRes PhoneCode(PhoneCode model)
        {
            ...
            string codeToken = CS.GetCacheKeySmsCode($"{phone}_{CS.SMS_TYPE.AUTH}");
#if !DEBUG
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        [HttpPost, Route("register/mchRegister"), MethodLog("商户注册")]
        public ApiRes Register(Register model)
        {
            string phone = Base64Util.DecodeBase64(model.phone);
            string code = Base64Util.DecodeBase64(model.code);
            string confirmPwd = Base64Util.DecodeBase64(model.confirmPwd);
            string codeToken = CS.GetCacheKeySmsCode($"{phone}_{CS.SMS_TYPE.REGISTER}");
        ...
        public ApiRes SendCode(SmsCode model)
        {
            ...
            if (model.smsType.Equals(CS.SMS_TYPE.RETRIEVE) && !_sysUserService.IsExistTelphone(model.phone, CS.SYS_TYPE.MCH))
            {
                throw new BizException("用户不存在！");
            }

            if (model.smsType.Equals(CS.SMS_TYPE.AUTH) && !_sysUserService.IsExistTelphone(model.phone, CS.SYS_TYPE.MCH))
            {
                throw new BizException("用户不存在！");
            }
