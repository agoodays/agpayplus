    ...
    <a-card>
      <div class="table-page-search-wrapper">
        <a-form layout="inline"class="table-head-ground">
          <div class="table-layer">
            ...
            <ag-text-up :placeholder="'码牌别名'" :msg="searchData.qrcAlias" v-model="searchData.qrcAlias" />
      ...
      >
        <template slot="topLeftSlot">
          <div>
            <a-button v-if="$access('ENT_DEVICE_QRC_ADD')" type="primary" icon="plus" @click="addFunc" class="mg-b-30">新建</a-button>
          </div>
        </template>
        <template slot="qrcIdSlot" slot-scope="{record}"><b>{{ record.qrcId }}</b></template> <!-- 自定义插槽 -->
...
const tableColumns = [
  {
    key: 'qrcId', // key为必填项，用于标志该列的唯一
    fixed: 'left',
    title: '码牌ID',
    scopedSlots: { customRender: 'qrcIdSlot' }
  },
  {
    key: 'qrcAlias',
    dataIndex: 'qrcAlias',
