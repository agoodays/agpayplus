  ...
  <div class="chart-card">
        ...
        <div class="amount-top">
          <a-skeleton active :loading="skeletonLoading" :paragraph="{ rows: 6 }">
            ...
            <p class="amount-pay-amount">
              {{ dayCount.payAmount.toFixed(2) }}
            </p>
            <div class="amount-list">
              <div>
                <p>成交笔数(笔)</p>
                <span>{{ dayCount.payCount }}</span>
              </div>
              <div>
                <p>退款金额(元)</p>
                <span>{{ dayCount.refundAmount.toFixed(2) }}</span>
              </div>
              <div>
                <p>退款笔数(笔)</p>
                <span>{{ dayCount.refundCount }}</span>
              </div>
            </div>
          </a-skeleton>
        </div>
    ...
    <div class="personal">
      <div>
        ...
        <div class="personal-line"></div>
        <a-skeleton active :loading="skeletonLoading" :paragraph="{ rows: 4 }">
          <div class="notice-section">
            <div class="notice-title">最新公告</div>
            <div v-if="noticeData.length" class="notice-list">
              <div
                v-for="noticeItem in noticeData"
                :key="noticeItem.articleId"
                class="notice-item"
                @click="handleNoticeClick(noticeItem)"
              >
                <a-tag :color="noticeItem.noticeFlag === 1 ? 'red' : 'blue'">
                  {{ noticeItem.noticeFlag === 1 ? '热门' : '公告' }}
                </a-tag>
                <span class="notice-content">{{ noticeItem.articleTitle }}</span>
                <span class="notice-date">{{ noticeItem.releaseTime }}</span>
              </div>
            </div>
...
/** 代理商简称 */
const shortName = computed(() => userStore.shortName)
...
async function fetchPayDayCount() {
  try {
    ...
    Object.assign(dayCount, res.dayCount)
  } catch (err) {
...
async function fetchNotice() {
  try {
    const res = await noticeApi.queryPage({ pageNumber: 1, pageSize: 5 })
    noticeData.value = res.records || []
  } catch (err) {
 ...
 * 点击公告跳转详情
 */
function handleNoticeClick(noticeItem) {
  if (noticeItem && noticeItem.articleId) {
    router.push({ path: '/sysArticle/view', query: { id: noticeItem.articleId } })
  }
}
