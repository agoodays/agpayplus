        ...
        <template #opSlot="{ record }">
          <ag-table-actions>
            <a-button v-if="hasPermission('ENT_PAY_ORDER_VIEW')" type="link" @click="handleDetail(record)"
              >详情</a-button
            >
            <a-button v-if="hasPermission('ENT_PAY_ORDER_REFUND')" type="link" style="color: red"
              v-show="record.state === 2 && record.refundState !== 2" @click="handleRefund(record)">退款</a-button>
          </ag-table-actions>
        </template>
      </ag-table>
    </a-card>
...
const handleExport = (params) => {
  orderApi.exportPayOrder(params)
}
