<template>
  <div class="ag-date-range-picker" :class="{ 'is-focused': isFocused }">
...
const hasValue = computed(() => {
  return !!selectedOption.value || (dateRange.value && dateRange.value.length > 0)
})
...
const shouldFloat = computed(() => {
  return isFocused.value || hasValue.value || !!props.label
})
...
const getDateRangeByOption = (option) => {
  ...
  if (option === 'pyToDate') {
    ...
    return [
      ...
      priorYear.year(now.year() - 1).month(now.month()).date(now.date()).endOf('day')
    ]
  }
  ...
  if (option === 'nextQuarter') {
    return [
      now.add(1, 'quarter').startOf('quarter'),
      now.add(1, 'quarter').endOf('quarter')
    ]
  }
  
  if (option === 'nextYear') {
    return [
      now.add(1, 'year').startOf('year'),
      now.add(1, 'year').endOf('year')
    ]
  }
